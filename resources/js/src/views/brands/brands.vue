<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <br>
    <add-new-brand  @update="onChildUpdate" :isSidebarActive="addNewDataSidebar" @closeSidebar="addNewDataSidebar = false" />
    <vs-table ref="table" multiple v-model="selected"  :data="brands">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ brands.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : brands.length }} of {{ brands.length }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
      ...
      <template slot="thead">
        <vs-th>Image</vs-th>
        <vs-th sort-key="title">Brand Title</vs-th>
        <vs-th>Action</vs-th>
      </template>
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <div class="demo-alignment">
                <vx-tooltip color="success" text="Edit Brand">
                  <vs-button color="success" @click="edit(tr.id)" type="border" icon-pack="feather" icon="icon-edit"></vs-button>
                </vx-tooltip>
                <vx-tooltip color="danger" text="Delete Brand">
                  <vs-button color="danger" @click="deleteBrand(tr.id)" type="border" icon-pack="feather" icon="icon-trash"></vs-button>
                </vx-tooltip>
              </div>
            </vs-td>
          </vs-tr>
    ...
    <vs-popup title="UpdateBrand" :active.sync="popupActive">
          <server-error v-show="errorActive" :alertActi="er_active" :data="SError" />

          <div :key= "index" v-for="(lang, index) in brand.title" >
              <vs-input v-validate="'required'" :label="'Brand Title '+index" :name="'title_'+index" v-model="brand.title[index]" class="w-full" />
              <span class="text-danger text-sm" v-show="errors.has('title_'+index)">{{ errors.first('title_'+index) }}</span>
          </div>
          ...
          <vs-button @click.prevent="updateBrand"  class="mr-6">Update Data</vs-button>
    </vs-popup>
  </div>
</template>

<script>
import AddNewBrand from './AddNewBrand.vue';
...
export default {
  ...
  methods: {
    ...
    edit(id){
      var _this = this
      this.brandId = id
      this.$http.get('/api/brands/'+id+'/edit')
    ...
    deleteBrand(id){
      var _this = this
      this.$http.delete('/api/brands/'+id)
      .then(function(response){
        _this.$vs.notify({
          title:'Success',
          ...
          thisIns.$http.post('api/brands/'+thisIns.brandId, thisIns.DataEdit ,config)
            .then(function (response){
                ...
                thisIns.$vs.notify({
                  ...
                  thisIns.getBrand(thisIns.currentx)
