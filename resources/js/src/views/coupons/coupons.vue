<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <br>
    <add-new-coupon  @update="onChildUpdate" :isSidebarActive="addNewDataSidebar" @closeSidebar="addNewDataSidebar = false" />
    <vs-table ref="table" multiple v-model="selected" :max-items="itemsPerPage" :data="coupons">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ coupons.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : coupons.length }} of {{ coupons.length }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
      ...
      <template slot="thead">
        <vs-th sort-key="name">coupon</vs-th>
        <vs-th sort-key="code">value</vs-th>
        <vs-th sort-key="PhoneCode">expire_date</vs-th>
        <vs-th sort-key="code">Used</vs-th>
        <vs-th>Action</vs-th>
      </template>
      <template slot-scope="{data}">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            <vs-td>
              <p class="product-name font-medium">{{ tr.coupon }}</p>
            </vs-td>
            <vs-td>
              <p class="product-category">{{ tr.value }}</p>
            </vs-td>
            <vs-td>
              <p class="product-price">{{ tr.expire_date }}</p>
            </vs-td>
            <vs-td>
              <p class="product-price">{{ tr.used }}</p>
            </vs-td>
            <vs-td>
              <div class="demo-alignment">
                <!-- <vx-tooltip color="success" text="Edit Coupon">
                  <vs-button color="success" @click="edit(tr.id)" type="border" icon-pack="feather" icon="icon-edit"></vs-button>
                </vx-tooltip> -->
                <vx-tooltip color="danger" text="Delete Coupon">
                  <vs-button color="danger" @click="deleteCoupon(tr.id)" type="border" icon-pack="feather" icon="icon-trash"></vs-button>
                </vx-tooltip>
    ...
    <vs-popup title="UpdateCoupon" :active.sync="popupActive">
          ...
          <vs-input-number v-validate="'required|min:1|max:1000'" label="Coupon Number" name="number" v-model="coupon.number" class="w-full" />
          <span class="text-danger text-sm" v-show="errors.has('number')">{{ errors.first('number') }}</span>

          <datepicker v-model="coupon.expire_date" format="yyyy-MM-dd"></datepicker>

          <vs-button @click.prevent="updateCoupon"  class="mr-6">Update Data</vs-button>
    </vs-popup>
  </div>
</template>

<script>
import AddNewCoupon from './AddNewCoupon.vue';
...
export default {
  ...
  methods: {
    ...
    edit(id){
      var _this = this
      this.couponId = id
      this.$http.get('/api/coupons/'+id+'/edit')
    ...
    deleteCoupon(id){
      var _this = this
      this.$http.delete('/api/coupons/'+id)
      .then(function(response){
        _this.$vs.notify({
          title:'Success',
          ...
          _this.getCoupon(_this.currentx)
          ...
          thisIns.$http.post('api/coupons/'+thisIns.couponId, thisIns.DataEdit ,config)
            .then(function (response){
                ...
                thisIns.$vs.notify({
                  ...
                  thisIns.getCoupon(thisIns.currentx)
