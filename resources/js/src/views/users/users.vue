<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <br>
    <add-new-user  @update="onChildUpdate" :isSidebarActive="addNewDataSidebar" @closeSidebar="addNewDataSidebar = false" />
    ...
    <vs-table ref="table" multiple v-model="selected" :max-items="itemsPerPage" :data="users">
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <div class="demo-alignment">
                <vx-tooltip color="success" text="Edit User">
                ...
                <vx-tooltip color="danger" text="Delete User">
                  <vs-button color="danger" @click="deleteUser(tr.id)" type="border" icon-pack="feather" icon="icon-trash"></vs-button>
                </vx-tooltip>
              </div>
            </vs-td>
          </vs-tr>
    ...
    <vs-popup title="UpdateUser" :min="1" :active.sync="popupActive">
          ...
          <vs-button @click.prevent="updateUser"  class="mr-6">Update Data</vs-button>
    </vs-popup>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ["Email", "Phone","Name"],
      headerVal: ["email", "phone","name"],
      fileName: "",
      activePrompt: false,
    }
  },
  ...
  methods: {
    ...
    edit(id){
      var _this = this
      this.userId = id
      this.$http.get('/api/users/'+id+'/edit')
          ...
          thisIns.$http.post('api/users/'+thisIns.userId, thisIns.DataEdit ,config)
            .then(function (response){
                ...
                thisIns.$vs.notify({
                  ...
                  thisIns.getUser(thisIns.currentx)
