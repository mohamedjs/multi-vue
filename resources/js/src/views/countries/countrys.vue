<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <br>
    <add-new-country  @update="onChildUpdate" :isSidebarActive="addNewDataSidebar" @closeSidebar="addNewDataSidebar = false" />
    <vs-table ref="table" multiple v-model="selected" :max-items="itemsPerPage" :data="countrys">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ countrys.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : countrys.length }} of {{ countrys.length }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="PhoneCode">PhoneCode</vs-th>
        <vs-th>Action</vs-th>
      </template>
      <template slot-scope="{data}">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="product-category">{{ tr.code }}</p>
            </vs-td>
            <vs-td>
              <p class="product-price">{{ tr.phonecode }}</p>
            </vs-td>
            <vs-td>
              <div class="demo-alignment">
                <!-- <vx-tooltip color="success" text="Edit Country">
                  <vs-button color="success" @click="edit(tr.id)" type="border" icon-pack="feather" icon="icon-edit"></vs-button>
                </vx-tooltip>
                <vx-tooltip color="danger" text="Delete Country">
                  <vs-button color="danger" @click="deleteCountry(tr.id)" type="border" icon-pack="feather" icon="icon-trash"></vs-button>
                </vx-tooltip> -->
                <vx-tooltip color="warning" text="Show State">
                  <vs-button color="warning" @click="showState(tr.id)" type="border" icon-pack="feather" icon="icon-play"></vs-button>
                </vx-tooltip>
              </div>
            </vs-td>
          </vs-tr>
    ...
    <vs-popup title="UpdateCountry" :active.sync="popupActive">
          <server-error v-show="errorActive" :alertActi="er_active" :data="SError" />
          <vs-input v-validate="'required'" label="Name" name="name" v-model="country.name" class="w-full" />
          <span class="text-danger text-sm" v-show="errors.has('name')">{{ errors.first('name') }}</span>

          <vs-input v-validate="'required'" label="Code" name="code" v-model="country.code" class="w-full" />
          <span class="text-danger text-sm" v-show="errors.has('code')">{{ errors.first('code') }}</span>

          <vs-input v-validate="'required'" label="PhoneCode" name="phonecode" v-model="country.phonecode" class="w-full" />
          <span class="text-danger text-sm" v-show="errors.has('phonecode')">{{ errors.first('phonecode') }}</span>

          <vs-button @click.prevent="updateCountry"  class="mr-6">Update Data</vs-button>
    </vs-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    edit(id){
      var _this = this
      this.countryId = id
      this.$http.get('/api/countrys/'+id+'/edit')
    ...
    deleteCountry(id){
      var _this = this
      this.$http.delete('/api/countrys/'+id)
      .then(function(response){
        _this.$vs.notify({
          title:'Success',
          ...
          _this.getCountry(_this.currentx)
          ...
          thisIns.$http.post('api/countrys/'+thisIns.countryId, thisIns.DataEdit ,config)
            .then(function (response){
                ...
                thisIns.$vs.notify({
                  ...
                  thisIns.getCountry(thisIns.currentx)
