<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected"  :data="products">
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ADD NEW -->
          <div class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between text-lg font-medium text-base text-primary border border-solid border-primary" @click="$router.push({name: 'product-add'})">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="product-title">Product Title</vs-th>
        <vs-th sort-key="stock">Stock</vs-th>
        <vs-th sort-key="min-stock">Min Stock</vs-th>
        <vs-th sort-key="special">Special</vs-th>
        ...
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            <vs-td class="img-container">
              <img :src="tr.main_image" class="product-img" />
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.category.title }}</p>
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.brand.title }}</p>
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.title }}</p>
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.stock }}</p>
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.min_stock }}</p>
            </vs-td>
            <vs-td>
              <vs-switch v-model='tr.special' @change="Uspecial(!tr.special)" />
            </vs-td>
            <vs-td>
              <vs-switch v-model='tr.active' @change="Uactive(!tr.active)" />
            </vs-td>
            <vs-td>
              <p class="product-name font-medium">{{ tr.price }}</p>
            </vs-td>
            <vs-td>
              <div class="demo-alignment">
                <vx-tooltip color="success" text="Edit Product">
                  <vs-button color="success" :to="{name: 'product-update', params: { productId: tr.id}}" type="border" icon-pack="feather" icon="icon-edit"></vs-button>
                </vx-tooltip>
                <vx-tooltip color="danger" text="Delete Product">
                  <vs-button color="danger" @click="deleteProduct(tr.id)" type="border" icon-pack="feather" icon="icon-trash"></vs-button>
                </vx-tooltip>
              </div>
            </vs-td>
          </vs-tr>
...
export default {
  ...
  data() {
    return {
      ...
      cellAutoWidth: true,
      selectedFormat: "xlsx",
      headerTitle: ['Title','category','brand','Price','Special','Active','Stock','Min Stock'],
      headerVal: ['title','category','brand','price','special','active','stock','min_stock'],
      fileName: "",
      activePrompt: false,
    }
  },
  ...
  methods: {
    ...
    edit(id){
      var _this = this
      this.productId = id
      this.$http.get('/api/products/'+id+'/edit')
    ...
    deleteProduct(id){
      ...
      .then(function(response){
        _this.$vs.notify({
          ...
          _this.getProduct(_this.currentx)
          ...
          thisIns.$http.post('api/products/'+thisIns.productId, thisIns.DataEdit ,config)
            .then(function (response){
                ...
                thisIns.$vs.notify({
                  ...
                  thisIns.getProduct(thisIns.currentx)
